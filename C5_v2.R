...
trainingData <- new_train
#testData <- new_train[-TrainingDataIndex,]
testData=new_test
TrainingParameters <- trainControl(method = "repeatedcv", number = 5, repeats=5)
DecTreeModel <- train(outcome_type ~ . -animal_id_outcome-age_upon_intake_age_group-intake_time
                      -intake_condition, data = trainingData, 
                      method = "C5.0",
...
solution <- data.frame('animal_id_outcome' = new_test$animal_id_outcome,'outcome_type'= DTPredictions)

# Write it to file
write.csv(solution, 'solution5.csv', row.names = F,quote = F)
