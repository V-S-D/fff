...
y_train <- as.numeric(new_train$outcome_type) - 1

# keep track of the labels
labels_train <- data.frame(new_train$outcome_type, y_train)

# xgboost-specific design matrices
xgb_train <- xgb.DMatrix(model.matrix(~.-animal_id_outcome-age_upon_intake_age_group-intake_time
                                      -intake_condition, data=new_train),
                         label=y_train, missing=NA)
xgb_test <- xgb.DMatrix(model.matrix(~.-animal_id_outcome-age_upon_intake_age_group-intake_time
                                     -intake_condition, data=new_test), missing=NA)

# tune nrounds - uncomment first
xgb.cv(data=xgb_train, label=y_train, nfold=5, nround=200, objective='multi:softmax',
       num_class=9, eval_metric='merror')

# looks like nrounds should be around 45


# build model
xgb_model <- xgboost(xgb_train, y_train, nrounds=45, objective='multi:softmax',
                     num_class=9, eval_metric='mlogloss',
